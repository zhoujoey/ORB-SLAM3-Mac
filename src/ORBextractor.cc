...
void ORBextractor::ComputeKeyPointsOctTree(
	vector<vector<cv::KeyPoint> >& allKeypoints)	//所有的特征点，这里第一层vector存储的是某图层里面的所有特征点，
...
void ORBextractor::ComputeKeyPointsOld(
	std::vector<std::vector<cv::KeyPoint> > &allKeypoints)		//输出，所有图层上的所有特征点
{
    ...
    for (int level = 0; level < nlevels; ++level)
    {
        ...
        for(int i=0; i<levelRows; i++)
        {
            ...
            for(int j=0; j<levelCols; j++)
            {
                ...
                cv::KeyPointsFilter::retainBest(keysCell,			//输入输出，用于提供待保留的特征点vector，操作完成后将保留的特征点存放在里面
